...
//dbus represents just the busname in this representation using rbus
    ...
    , mHandlerId(1)
{
    ...
    // Assuming this doesn't need to happen for rbus - won't be used with SkyQ devices
    // which this pertains to.
...
DobbyIpcBus::~DobbyIpcBus()
{
    ...
    // Close thread stuff - not relevant rbus
    /*
...
std::string DobbyIpcBus::socketPathFromAddress(const std::string& address)
{
    ...
    if (address.empty())
    {
        AI_LOG_WARN("ATTEMPTED TO RETRIEVE SOCKET PATH");
        return std::string();
    }
...
bool DobbyIpcBus::connect(const std::string& dbusAddress)
{
    ...
    /* --- 
        Might be dbus specific - might not need signal handling? (change to event?)
    */
